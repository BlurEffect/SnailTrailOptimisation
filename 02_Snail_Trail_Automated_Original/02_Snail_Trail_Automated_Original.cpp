/*
02_Snail_Trail_Automated_Original
Kevin Meergans, 2013, based on the snail trail game by A. Oram
This version of the snail trail automatically replays games previously recorded with version 01. The user input is
read in from a file "Keys.txt" and the game records the time each frame of the game (iteration of the game loop) takes
to complete. The code allows for multiple runs through the pre-recorded input and accumulates the frametimes for
the respective frames (i.e. all frametimes for the first frame of the recorded input are accumulated and so on). 
The accumulated times are written to a file "Framerates.txt", from where they can be imported to Excel. All further
version build upon this version. In this version, no changes have been made to the game code except for the 
code required to execute the actions described above (reading keys, accumulating times, writing data to file).
*/
...
int main()
{
	...
	// write the recorded times to a file
	for(int k = 0; k < 120; ++k)
	{
