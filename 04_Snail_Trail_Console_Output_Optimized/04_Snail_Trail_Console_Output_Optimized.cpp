/*
04_Snail_Trail_Console_Output_Optimized
Kevin Meergans, 2013, based on the snail trail game by A. Oram
For this version, the console output has been optimized by using alternate methods to the original use of cout.
The version only prints contents to the console that have been changed compared to the previous frame, instead
of printing everything to the console each frame. To determine changes that require reprinting additional variables
and a queue data structure were introduced. The main changes can be seen in the paintGame function and the functions
that are called from there. Additionally, all other couts have been replaced with calls to printf.
*/
...
// required for new output method

// stores which characters have to be replaced in the next screen update;
// is filled throughout the code when changes occur (snail/frogs moving, new slime,...)
queue<pair<COORD, char>> changeQueue;
// store the amount of pellets of the last frame to determine whether a new one was collected
int pelletCount = 0;
// store the active message of the last frame to compare it with the current one
string lastMessage = " ";
...
int main()
{
	...
	for(int i = 0; i < numberOfCycles; ++i)
	{
		...
		{	
			...
			while (((key | 0x20) != QUIT) && snailStillAlive && !fullOfLettuce)	//user not bored, and snail not dead or full (main game loop)
			{
				...
				//outFramerates << s.getElapsedTime() << ",";
				accumulatedFramerates[iterationCount] += s.getElapsedTime();
	...
	for(int k = 0; k < 120; ++k)
	{
		outFramerates << accumulatedFramerates[k] << ",";
	}
...
{ //display game title, messages, snail & other elements on screen
	...
	{
		// while a game is running, only reprint the stuff that has changed
...
{ 
	...
	// having to fill the buffer each call is a caveat.
...
{
	// Print the necessary changes to the garden
